...
// 1. Tentukan Props untuk modal
interface ImportPresensiModalProps {
    open: boolean;
    onOpenChange: (open: boolean) => void;
}

export default function ImportPresensiModal({ open, onOpenChange }: ImportPresensiModalProps) {
    ...
    const handleSubmit = (e: FormEvent) => {
        e.preventDefault();
        post(route("presensi.import.store"), {
            onSuccess: () => {
                onOpenChange(false);
                reset();
            },
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-lg">
                <DialogHeader>
                    <DialogTitle className="text-xl font-semibold">Import Presensi dari CSV</DialogTitle>
                    <DialogDescription>
                        Unggah file CSV untuk membuat data presensi secara massal.
                    </DialogDescription>
                </DialogHeader>
